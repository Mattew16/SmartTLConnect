...
public class ExtIFServer implements ExternalIf {
  ...
  //int[] statusUserChange indicates if there is a user forced change on the status of the intersection.
  public static int[] statusUserChange = {0};

  //int[] status stores the status of each intersection. 
  ...
  */
  public ExtIFServer() {
    ...
    //Initialization of empty ObjectEventBuffers.
    ...
    // fill some initial data to represent the objects; usually the event buffer is updated during runtime 
    ...
    //TODO make a seamless cycle for any number. (requires user input to the number of expected intersections, also with the configuration command to add or remove intersections)
    ...
    //TODO creates an intersection and puts it in the buffer, changing all the relevant variables to accommodate this change.
  public void createIntersections(){
    Scanner reader = new Scanner(System.in);  // Reading from System.in
    System.out.println("Enter the number of intersections: ");
    int numIntersections = reader.nextInt(); // Scans the next token of the input as an int.
    int i;
    ...
    for(i=0;i<numIntersections;i++){
      ...
      previousStatusControl[i].setSpNr((short) 0);
    }
