...
public class ExtIFServer implements ExternalIf
{
    ...
    public ExtIFServer()
    {
        ...
        String[] intersectionids = { "Intersection_01"};
        ...
        for (String intersectioId : intersectionids)
        {
          ...
          co.setX(5220.41749);      // some position in UTM zone 29, use right UTM position here
          co.setY(-4530.30618);
          LocationTypeCo_description coDesc = new LocationTypeCo_description();
          coDesc.setProjectionCategory("UTM WGS84");
          coDesc.setProjectionType("zone=32,band=U"); // zone 29 = Portugal band = N / 31 Netherlands band = U
        ...
        for (String intersectioId : intersectionids)
        {
          ...
          FaultStatusType fs = new FaultStatusType((short) status, null, null);
    ...
    public PutResponseType put(PutType put) throws RemoteException
    {
        ...
        else
        {
            for (int i = 0; i < putds.length; i++)
            {
                ...
                if (isCommand.getSignalProgram().getSpNr() != status){
                  status = isCommand.getSignalProgram().getSpNr();
                  statusUserChange = 1;
                }
    ...
    public static void main(String[] args) throws IOException, InterruptedException
    {
        ...
        int count =0, previousStatus = 9, delay = 0;
        SpType previousStatusControl = new SpType();
        previousStatusControl.setSpNr((short) 1);
        String[] intersectionids = { "Intersection_01"};
        ...
        for (int i = 0; i < 500000; i++)
            {
              ...
              delay++;
              if ((status != previousStatus) && (statusUserChange == 1)){
              ...
              if(status == 9 && delay != 2){
                ...
                previousStatus = status;
                previousStatusControl.setSpNr((short) previousStatus);
                ...
                for (String intersectioId : intersectionids){
                  ...
                  //SpType sp = new SpType();
                  //sp.setSpNr((short) 1);
                  isStatus.setSignalProgram(previousStatusControl);
                  // now we have the minimum required values
                  intersectionStatusBuffer.writeObject(objIdent, isStatus, false);
